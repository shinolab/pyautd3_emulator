...
def download_and_extract(repo: str, name: str, version: str) -> None:
    ...
    dst_dir = Path("pyautd3_emulator/bin")
...
def should_update_dll(version: str) -> bool:
    if platform.system() == "Windows":
        if not Path("pyautd3_emulator/bin/autd3capi_emulator.dll").exists():
            return True
    elif platform.system() == "Darwin":
        if not Path("pyautd3_emulator/bin/libautd3capi_emulator.dylib").exists():
            return True
    elif not Path("pyautd3_emulator/bin/libautd3capi_emulator.so").exists():
        return True
...
def copy_dll() -> None:
    with Path("pyautd3_emulator/__init__.py").open() as f:
        content = f.read()
